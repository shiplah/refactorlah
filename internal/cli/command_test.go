...
func TestApplyGoPackageMoveUpdatesPackageReferences(t *testing.T) {
	root := plainProject(t)
	mustWriteFile(t, filepath.Join(root, "go.mod"), "module github.com/shiplah/refactorlah\n")
	mustWriteFile(t, filepath.Join(root, "internal", "adapters", "php", "parser.go"), `package php

import "github.com/shiplah/refactorlah/internal/parsing/treesitter"
...
`)
	...
	if !strings.Contains(parser, `"github.com/shiplah/refactorlah/internal/parsing/document"`) {
...
import "github.com/shiplah/refactorlah/internal/models"
