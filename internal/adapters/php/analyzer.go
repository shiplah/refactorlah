...
func (a *Analyzer) collectReplacements(projectRoot string, composerRoot string, mappings []adapterproto.SymbolMapping, scanIndex *scan.Index) ([]adapterproto.Replacement, []adapterproto.Warning, error) {
	...
	for _, phpFile := range phpFiles {
		...
		if mapping, ok := movedFiles[phpFile]; ok {
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.namespaceRule.Collect(document, rules.NamespaceDeclarationInput{
				File:         phpFile,
			...
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.importRemovalRule.Collect(document, rules.SameNamespaceImportRemovalInput{
				File:         phpFile,
				Source:       source,
				NewNamespace: mapping.NewNamespace,
				Mappings:     mappingReferences,
		...
		if movedMapping, ok := movedFiles[phpFile]; ok {
			...
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.sameNamespaceImportRule.Collect(document, rules.SameNamespaceReferenceImportInput{
				File:     phpFile,
				Source:   source,
				Mappings: mappingReferences,
			}))...)
		...
		for _, mapping := range mappings {
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.useStatementRule.Collect(document, rules.UseStatementInput{
				File:                          phpFile,
				OldSymbol:                     mapping.OldSymbol,
				NewSymbol:                     mapping.NewSymbol,
				SameNamespaceRemovalNamespace: sameNamespaceRemovalNamespace,
			}))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.fqcnRule.Collect(document, rules.FullyQualifiedClassNameInput{
				File:      phpFile,
				OldSymbol: mapping.OldSymbol,
				NewSymbol: mapping.NewSymbol,
			}))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.classConstantRule.Collect(document, rules.ClassConstantInput{
				File:      phpFile,
				OldSymbol: mapping.OldSymbol,
				NewSymbol: mapping.NewSymbol,
			}))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.shortNameRule.Collect(document, rules.ShortClassNameReferenceInput{
				File:      phpFile,
				Source:    source,
				OldSymbol: mapping.OldSymbol,
				NewSymbol: mapping.NewSymbol,
			...
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.docblockVarRule.Collect(document, symbolInput))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.docblockParamRule.Collect(document, symbolInput))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.docblockReturnRule.Collect(document, symbolInput))...)
			allReplacements = append(allReplacements, shared.ToAdapterReplacements(a.docblockThrowsRule.Collect(document, symbolInput))...)
		}
